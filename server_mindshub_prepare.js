...
N’invente pas de statistiques, de blessures, de résultats ou de cotes en direct. Ne donne aucune longue explication. Utilise exactement cette structure :
{
  ...
  try {
    ...
    // MindsHub est prioritaire. L’URL est configurable pour éviter
    // de modifier le reste de l’application si MindsHub change son endpoint.
    const MINDSHUB_API_URL =
      process.env.MINDSHUB_API_URL ||
      "https://api.mindshub.ai/v1/chat/completions";

    if (process.env.MINDSHUB_API_KEY) {
      const response = await fetch(MINDSHUB_API_URL, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          model: process.env.MINDSHUB_MODEL,
          temperature: 0.2,
          messages: [
      ...
      if (!response.ok) {
        console.error(
          "MindsHub provider error:",
          response.status,
          data?.error?.message || "unknown"
        );
        return res.status(502).json({
          error: "Le service MindsHub est temporairement indisponible."
        });
      }
      ...
      return res.status(503).json({
        error: "Configurez MINDSHUB_API_KEY et MINDSHUB_MODEL pour activer BatBot IA."
      });
    }
